...
public class TaskTraceReader implements TaskTraceReaderInterface {
    ...
    public TaskTraceReader(Task2 task, long taskStart, WildcardPattern filter, String[] files, WildcardPattern tracePattern, boolean cached) throws IOException {
    ...
    public DataPacket getNextPacket(DataPacket p) throws DataPacket.PacketParseException, IOException {
        while (true) {
            ...
            long packetField = task.getPacketField(p);
            packetField = (packetField >> (WildcardPattern.TOTAL_LENGTH - tracePattern.getWildcardNum())) + tracePatternShift;
            task.setPacketField(p, packetField);
