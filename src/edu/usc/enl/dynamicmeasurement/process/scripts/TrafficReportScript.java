...
//        }

        {
            //create traffic report user
            int wildcard = 0;
            PacketUser user = new SkewFinderReport(wildcard);
            ((SkewFinderReport) user).setStatsOutputWriter(new PrintWriter("output/skew" + "_" + wildcard + ".txt"));
            user = new EpochPacer(user, 1000000);
            //run simulator
            new Simulator(0).run(packetFiles, user);
        }
...
//            monitors.add(new WildcardPattern("0010____________________________",0));
//            monitors.add(new WildcardPattern("0011____________________________",0));
////            int items = 16;
