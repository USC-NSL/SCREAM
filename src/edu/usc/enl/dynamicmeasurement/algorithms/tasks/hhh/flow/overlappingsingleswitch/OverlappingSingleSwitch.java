...
public class OverlappingSingleSwitch extends FlowHHHAlgorithm implements SingleSwitchTask.SingleSwitchTaskImplementation {
    ...
    public OverlappingSingleSwitch(Element element) {
        ...
        if (startLevel < getTaskWildcardPattern().getWildcardNum()) {
            unusedCapacity = capacity - (1 << startLevel);
        } else {
            startLevel = getTaskWildcardPattern().getWildcardNum();
            unusedCapacity = 0;
        }
//        int unusedCapacity = capacity - (1 << startLevel);
        int preliminaryLevel = getTaskWildcardPattern().getWildcardNum() - startLevel;
        long baseData = getTaskWildcardPattern().getData() << startLevel;
