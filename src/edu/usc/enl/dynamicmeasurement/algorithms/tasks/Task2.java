...
//        filter = new WildcardPattern(childrenProperties.get("Filter").getAttribute(ConfigReader.PROPERTY_VALUE), 0);
    ...
    public WildcardPattern getFilter() {
        return user.getImplementation().getTaskWildcardPattern();
    }
    ...
    public long getPacketField(DataPacket p) {
        try {
            return (long) user.getGetKeyField().invoke(p);
    ...
    public void process(DataPacket p) {
        WildcardPattern filter = getFilter();
//        if (name.equals("5")) {
//            System.out.println(getName() + "," + new WildcardPattern(p.getDstIP(), 0, 0).toStringNoWeight() + ", " + filter);
//        }
        if (filter.match(getPacketField(p))) {
            synchronized (this) {
...
//        System.out.println("Su " + getName());
    ...
    public void setPacketField(DataPacket p, long l) {
        try {
            user.setKeyField.invoke(p, l);
        } catch (IllegalAccessException | InvocationTargetException e) {
            e.printStackTrace();
        }
    }
